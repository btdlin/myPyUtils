...
def test_global_filter():
    candidates = CLIENT.get_candidates('green-rush-daily', filter=f.named_filter('GLOBAL'), limit=1000, sort_by=SortStrategy.POP_1D)
    #print(candidates)
...
def test_metafilter_resource_type_article():
    ...
    candidates = CLIENT.get_candidates('green-rush-daily', filter=f.and_filter(f.named_filter('GLOBAL'), filter), limit=1000, sort_by=SortStrategy.POP_1D)
